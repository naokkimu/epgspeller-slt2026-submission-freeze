...
```json
{
  ...
  "pdf": {
    ...
    "sha256": "ec0d2bd657491887bd916a1ca0603019dcb94df6b72df4a21a3e75aaf5854043",
    "sha8": "ec0d2bd6",
    "truncated": false
  },
  ...
  "tex": {
    ...
    "sha256": "75141a7521f4b6d3148a1e97550934f07c4322237c0ff2db1e0cb792e31cc964",
    "sha8": "75141a75"
  }
}
```
...
```json
{
  "checks": [
    ...
    },
    {
      ...
      "id": "abstract_length",
      "message": "abstract_chars=821 <= max_characters=1000",
      "ok": true,
      "severity": "info"
    },
    {
  ...
  "run_id": "INTSP2026_STATIC_ec0d2bd6_75141a75"
}
```
...
silent spelling from binary palate contact patterns. We
define protocols for word holdout, instance holdout,
and cross participant transfer. Using four participants
and deterministic artifact tracking, we evaluate a vec-
tor baseline and two layout aware front ends. We also
analyze electrode reduction and how models adapt in
low shot settings. Across our audited runs, the row
and column front end tracks the vector baseline more
closely than a convolutional grid front end. The grid
front end increases streaming latency. We release split
manifests, checksums, and compact result tables as
...
dataset; id is the participant identifier, N is sample
count, V is vocabulary size, Tmed is median sequence
length, contact is mean contact rate, and zero is the
count of all zero samples.
...
label counts, and anomalies before we construct any
train and test splits. We exclude a small set of all-zero
samples via a pinned index list.
...
vocabulary constraint. When a target participant has
limited within word repetition, we configure the cross
participant split generators to allow single instance
target words. We keep the source side constraints
unchanged. All split archives used in this study are
enumerated with sizes and checksums in manifest
artifacts.
...
show the word-holdout protocol uses all four par-
ticipant datasets. They show the instance-holdout
protocol uses three participant datasets. They also
show the fourth participant appears only as a target
in the cross-participant splits.
...
tocols; the protocol column denotes word holdout,
instance holdout, and cross participant transfer, CER
is character error rate, and lex is lexicon projected
error rate.
protocol n cer lex
...
convolutional feature extraction. We augment spa-
tially by perturbing contiguous electrode blocks to
emulate missing contacts and minor spatial shifts.
...
cross participant transfer. The n value counts the
split aggregates used per protocol in the metrics reg-
istry. Lexicon projection reduces error rates relative
to greedy decoding across protocols. This highlights
the importance of open-vocabulary post processing
for spelling.
We also evaluate cross participant transfer targeting
participant four under both single source and multi
source settings. The table uses lvl labels dir for single
directions and all for the across direction aggregate.
The corresponding split archives are pinned by check-
sum in a dedicated manifest artifact. Group labels use
p with source identifiers and an arrow to the target,
Table 3: Cross participant generalization targeting
participant four; proto distinguishes single source and
multi source cross participant protocols, lvl marks
direction level or across direction aggregate, group
encodes source to target participant identifiers, and
...
and paired sources concatenate. In our audited splits,
the multi source aggregate reduces CER relative to
the single source aggregate for the same target.
...
tive bias variants at full channels across protocols. We
include a patchpool grid encoder as a minimal way
to implement a rescue for the grid encoder. The vari-
ant labels are vec, rowcol, grid, grid aug, patch, and
patch aug. The row and column front end tracks the
vector baseline more closely than the convolutional
grid front end under within participant protocols.
Patchpool reduces the gap under cross participant
transfer. Across protocols, grid variants tend to in-
crease real time factor relative to the vector baseline.
We do not observe a consistent accuracy gain over the
vector baseline in any protocol.
...
This suggests that a compact subset can preserve
a large fraction of the vector baseline performance.
3

=== PAGE 4 ===
Table 4: Spatial modeling at full channels across
protocols; vec, rowcol, grid, grid aug, patch, and
patch aug denote vector baseline, row and column
pooling, convolutional grid encoder, grid with spatial
augmentation, patchpool grid encoder, and patchpool
with augmentation; CER is character error rate and
...
Random selection and simple transfer remain slightly
worse than within participant selection in the audited
results.
...
topk, fps two k, xfer, and rand denote within top
ranked selection, farthest point sampling, transfer
selection, and random selection; CER is character
...
varies within a narrow range across groups.
...
source pair; group concatenates source identifiers with
an arrow to the target, and vec, rowcol, grid, and grid
aug denote vector baseline, row and column pooling,
and grid encoder with or without spatial augmenta-
tion; CER is character error rate and RTF is real time
factor.
...
tion under limited participant coverage. We report
these trends as artifact grounded observations rather
than broad claims. We expect additional participant
data to be necessary for stronger transfer conclusions.
...
ticipant; group uses the participant identifier and k
one or k two for the number of training instances
per word, and vec, rowcol, grid, and grid aug denote
vector baseline, row and column pooling, and grid
encoder with or without spatial augmentation; CER
