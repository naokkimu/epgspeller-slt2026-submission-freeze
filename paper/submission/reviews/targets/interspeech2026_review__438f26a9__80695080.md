...
```json
{
  ...
  "pdf": {
    ...
    "sha256": "438f26a9f23b1cb59df5318ffa2921a52c727be67c5afba3d116e1c8d8286ca9",
    "sha8": "438f26a9",
    "truncated": false
  },
  ...
  "tex": {
    ...
    "sha256": "80695080631fab94ed8e41fd5edcab509cb18217c8492fa534cf5f42dc3269db",
    "sha8": "80695080"
  }
}
```
...
```json
{
  ...
  "run_id": "INTSP2026_STATIC_438f26a9_80695080"
}
```
...
We visualize how error changes as the electrode budget176
varies. 177
...
and the slope varies by selection strategy.179
...
ends.183
We evaluate low-shot adaptation by varying the number of184
training instances per word for a new participant.185
Two-shot generally improves over one-shot, while layout-186
aware variants remain mixed relative to the vector baseline.187
5. Discussion188
Across protocols, layout-aware front ends do not consistently189
outperform the vector baseline and often increase streaming190
cost; the patchpool grid reduces but does not eliminate the gap.191
Cross-participant transfer remains the main bottleneck. Source192
aggregation and low-shot adaptation help in some settings, but193
outcomes are mixed, and simple similarity indicators do not194
yield a single rule for when multi-source helps. Design implica-195
tion: when low latency is the priority, the vector or row/col front196
ends are safer choices; when robustness to electrode dropout197
or spatial priors is desired, grid-based options require careful198
tuning and validation. These takeaways depend on the audited199
datasets, split protocols, and evaluation pipeline, and they may200
not transfer to other sensors, labeling schemes, or participant201
populations.202
6. Artifacts and Auditability203
This repository uses a strict paper registry that pins every ev-204
idence file by checksum and rejects unsupported manuscript205
blocks. The split manifests, aggregated metrics, compact ta-206
bles, statistical summaries, and analysis reports referenced in207
this paper are stored as deterministic artifacts, enabling audit208
...
Figure 1:Greedy error versus electrode budget in an audited
run set.
and reproduction within our repository environment. A supple-209
mentary archive includes the compact statistical summary table210
for reviewers. 211
7. Ethics and Disclosure212
We report results only for audited artifacts and do not claim213
broader demographic coverage. The datasets and splits used in214
this study are derived from participant recordings, and we focus215
on methodological clarity and artifact traceability rather than216
deployment claims. 217
8. Generative AI Use Disclosure218
We used generative AI tools for language polishing and format-219
ting support, and we verified technical claims against audited220
artifacts. 221
