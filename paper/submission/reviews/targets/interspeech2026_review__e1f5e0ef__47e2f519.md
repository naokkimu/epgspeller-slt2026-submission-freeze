...
```json
{
  ...
  "pdf": {
    ...
    "sha256": "e1f5e0ef17eb990d1f3306fa78f376e958bddb263c7ec5b29ea2a334ed41d9a9",
    "sha8": "e1f5e0ef",
    "truncated": false
  },
  ...
  "tex": {
    ...
    "sha256": "47e2f5194c2a2cfdac3f326d5f6a6a15d47aa58ce534815863fe3d8e6ce387c7",
    "sha8": "47e2f519"
  }
}
```
...
```json
{
  ...
  "run_id": "INTSP2026_STATIC_e1f5e0ef_47e2f519"
}
```
...
Table 1 summarizes baseline performance across the three155
...
evaluation protocols. 156
Table 1 shows a clear protocol ordering: CER is 0.145 to157
0.180 for within-participant protocols (P2 and P1) and 0.691158
for cross-participant P3, while LEX rises from 0.075 to 0.102159
up to 0.644. Transfer difficulty, not within-participant decoding,160
drives the largest residual error.161
Table 2 compares vector, row/col, and grid-based front ends162
at full channels across protocols.163
From Table 2, rowcol tracks vec across protocols (P1: 0.20164
vs 0.18, P2: 0.16 vs 0.15, P3: 0.68 vs 0.69 CER). Grid and165
grid aug are usually higher in CER and RTF, and patch variants166
only partially close this gap. The compact statistical summary167
in supplementary material shows broad interval overlap, so we168
treat these as directional trends rather than universal winners.169
Table 3 evaluates cross-participant transfer with participant170
p4 as target under single-source and paired-source settings.171
Table 3 shows an aggregate gain: CER drops from172
0.686±0.114 in P3 all-¿p4 to 0.582±0.064 in P3MS all-¿p4, and173
LEX drops from 0.659±0.073 to 0.554±0.034. Direction rows174
remain heterogeneous, so source-pair selection stays condition175
dependent. 176
Table 4 evaluates low-shot adaptation by varying the num-177
ber of training instances per word for a new participant.178
From Table 4, increasing from k1 to k2 improves vec179
(0.350-¿0.246) and rowcol (0.334-¿0.278), but worsens grid180
...
Table 5 evaluates reduced-channel subsets across protocols183
using within-participant, transfer, and random selection strate-184
gies.185
...
Table 5 shows small method deltas at K=64: P1 CER ranges186
0.191-0.205, P2 ranges 0.154-0.158, and P3 ranges 0.644-187
0.657. Because these ranges overlap strongly, we avoid a uni-188
versal ranking among topk, fps2k, xfer, and rand.189
Fig. 1 visualizes how accuracy and streaming cost vary with190
electrode budget for the word holdout protocol.191
0.10
0.15
0.20
CER
...
Fig. 1 shows CER improving as K increases toward full192
channels, while RTF changes less steeply. Random subsets193
remain below within-participant selections across most K, al-194
though the absolute gap narrows at larger budgets.195
Table 6 evaluates paired-source cross-participant transfer196
under the same target and protocol constraints.197
...
Table 6 is target dependent: for p23-¿p1, vec 0.473 and198
rowcol 0.476 outperform grid 0.602 and grid aug 0.691, while199

=== PAGE 4 ===
for p12-¿p3, grid 0.736 outperforms vec 0.838 and rowcol200
