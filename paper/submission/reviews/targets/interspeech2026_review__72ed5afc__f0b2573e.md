...
```json
{
  ...
  "pdf": {
    ...
    "sha256": "72ed5afcedb0ce9d878fe06d70459a5c098c61d85d265a09f1e4686c38731af9",
    "sha8": "72ed5afc",
    "truncated": false
  },
  ...
  "tex": {
    ...
    "sha256": "f0b2573ee695bae98de98b012b9973662e56330bc3ef4714162636956614e4b5",
    "sha8": "f0b2573e"
  }
}
```
...
```json
{
  ...
  "run_id": "INTSP2026_STATIC_72ed5afc_f0b2573e"
}
```
...
The audited label histogram for the fourth participant is dom-104
inated by single-instance labels, so we restrict that participant105
to target-only use in cross-participant evaluation. This separa-106
tion explains why the dataset summary includes all participant107
datasets even when a protocol uses only a subset.108
We normalize labels by uppercasing and filtering to alpha-109
bet characters, then represent each label as a space separated110
character sequence for CTC training and greedy decoding. We111
apply this step consistently during split construction and dataset112
preparation to avoid vocabulary drift.113
We keep the label normalization step consistent across all114
splits and dataset preparation steps. This aligns the decoder115
vocabulary with the audited labels and reduces drift between116
training and artifacts used to evaluate.117
We report character error rate from greedy decoding and118
streaming speed using real time factor. We define real time fac-119
tor as total inference time divided by total input duration. For120
character-level decoding we also report lexicon projection error121
rates using a training lexicon and a full lexicon.122
Greedy decoding reports unconstrained character se-123
quences. Lexicon projection maps outputs to finite word sets124
derived from the training vocabulary or the full audited lexicon.125
This allows us to separate decoding quality from lexicon con-126
straints. We compute real time factor by dividing inference time127
by input duration under the same test harness. Tables summa-128
rize results over four split seeds as mean and standard devia-129
tion. We compute two sided t distribution confidence intervals130
for the primary accuracy metric and paired t tests with Holm131
correction; the compact statistical summary table is provided in132
the supplementary archive.133
3. Models134
Our baseline model encodes each frame as a vector of palate135
channels and applies a uni-directional recurrent decoder trained136
with a CTC objective. We compare two layout aware front ends.137
One is a row and column pooling front end that aggregates a138
proxy grid into one dimensional summaries. The other is a grid139
front end that reconstructs a proxy grid and applies a convolu-140
tional spatial encoder. For the grid model we optionally enable a141
spatial augmentation that drops and shifts contiguous electrode142
blocks. [11, 19]143
The proxy grid is constructed from the palate channel lay-144
...
out. It supports row and column pooling or convolutional fea-145
ture extraction. We use a fixed layout file for the mapping so146
that row and column indices are consistent across splits. We147
augment spatially by perturbing contiguous electrode blocks to148
emulate missing contacts and minor spatial shifts.149
We fix core training hyperparameters across runs and record150
how we configure them in the metrics registry for auditability.151
4. Results152
We report greedy decoding error, lexicon-projected error, and153
streaming cost under a shared evaluation harness for all proto-154
cols. All metrics are derived from the same audited registry.155
We evaluate cross-participant transfer with the fourth par-156
ticipant as target under single-source and paired-source settings.157
Paired-source transfer is often lower error than single-158
source transfer, but direction-level outcomes are mixed.159
We compare vector, row/col, and grid-based front ends at160
full channels across protocols.161
Row/col tracks the vector baseline more closely than grid162
variants. Grid variants tend to worsen accuracy and increase163
latency, and patchpool reduces but does not remove this gap.164
We evaluate reduced-channel subsets using within-165
participant, transfer, and random selection strategies.166
Within-participant selection is similar to the vector base-167
line, while transfer and random subsets are generally lower and168
mixed across protocols.169
...
ends. 173
We evaluate low-shot adaptation by varying the number of174
training instances per word for a new participant.175
Two-shot generally improves over one-shot, while layout-176
aware variants remain mixed relative to the vector baseline.177
5. Discussion178
Across protocols, layout-aware front ends do not consistently179
outperform the vector baseline and often increase streaming180
cost; the patchpool grid reduces but does not eliminate the gap.181
Cross-participant transfer remains the main bottleneck. Source182
aggregation and low-shot adaptation help in some settings, but183
outcomes are mixed, and simple similarity indicators do not184
yield a single rule for when multi-source helps. Design implica-185
tion: when low latency is the priority, the vector or row/col front186
...
ends are safer choices; when robustness to electrode dropout187
or spatial priors is desired, grid-based options require careful188
tuning and validation. These takeaways depend on the audited189
datasets, split protocols, and evaluation pipeline, and they may190
not transfer to other sensors, labeling schemes, or participant191
populations.192
6. Artifacts and Auditability193
This repository uses a strict paper registry that pins every ev-194
idence file by checksum and rejects unsupported manuscript195
blocks. The split manifests, aggregated metrics, compact ta-196
bles, statistical summaries, and analysis reports referenced in197
this paper are stored as deterministic artifacts, enabling audit198
and reproduction within our repository environment. A supple-199
mentary archive includes the compact statistical summary table200
for reviewers.201
7. Ethics and Disclosure202
We report results only for audited artifacts and do not claim203
broader demographic coverage. The datasets and splits used in204
this study are derived from participant recordings, and we focus205
on methodological clarity and artifact traceability rather than206
deployment claims.207
8. Generative AI Use Disclosure208
We used generative AI tools for language polishing and format-209
ting support, and we verified technical claims against audited210
artifacts.211
