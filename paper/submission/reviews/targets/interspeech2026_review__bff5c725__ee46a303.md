...
```json
{
  ...
  "pdf": {
    ...
    "sha256": "bff5c725ca7b6bf57fc86ff5725aa6b73088f8599eb6ee4357bb9380ceaced0e",
    "sha8": "bff5c725",
    "truncated": false
  },
  ...
  "tex": {
    ...
    "sha256": "ee46a303adfce9f5519a2516a0cd8863ce935c3201749bfbcd1fa3e2be2f0bdf",
    "sha8": "ee46a303"
  }
}
```
...
```json
{
  ...
  "run_id": "INTSP2026_STATIC_bff5c725_ee46a303"
}
```
...
Table 4 evaluates cross-participant transfer with participant175
p4 as target under single-source and paired-source settings.176
Table 4 shows an aggregate gain: CER drops from177
0.686±0.114 in P3 all-¿p4 to 0.582±0.064 in P3MS all-¿p4, and178
LEX drops from 0.659±0.073 to 0.554±0.034. Direction rows179
remain heterogeneous, so source-pair selection stays condition180
dependent.181
Table 5 evaluates low-shot adaptation by varying the num-182
ber of training instances per word for a new participant.183
From Table 5, increasing from k1 to k2 improves vec184
(0.350-¿0.246) and rowcol (0.334-¿0.278), but worsens grid185
(0.515-¿0.616) and grid aug (0.611-¿0.689). Low-shot gain is186
architecture dependent, not automatic.187
Table 6 evaluates reduced-channel subsets across protocols188
using within-participant, transfer, and random selection strate-189
gies.190
...
Table 6 shows small method deltas at K=64: P1 CER ranges191
0.191-0.205, P2 ranges 0.154-0.158, and P3 ranges 0.644-192
0.657. Because these ranges overlap strongly, we avoid a uni-193
versal ranking among topk, fps2k, xfer, and rand.194
Fig. 1 visualizes how accuracy and streaming cost vary with195
electrode budget for the word holdout protocol.196
0.10
0.15
0.20
CER
...
Fig. 1 shows CER improving as K increases toward full197
channels, while RTF changes less steeply. Random subsets are198
generally above within-participant selections in CER, although199
the gap narrows at larger budgets.200
Table 7 evaluates paired-source cross-participant transfer201
under the same target and protocol constraints.202
...
Table 7 is target dependent: for p23-¿p1, vec 0.473 and203
rowcol 0.476 outperform grid 0.602 and grid aug 0.691, while204
for p12-¿p3, grid 0.736 outperforms vec 0.838 and rowcol205
0.790. Multi-source benefit is therefore conditional rather than206
universal.207
5. Discussion208
